...
namespace The_Blogs_Of_The_U.Infrastructure.MySqlDb.Repository
{
    public class MySqlRepositoryClient : IMySqlRepositoryClient
    {
        ...
        public async Task<List<Producto>> GetAllProductos()
        {
            ...
            var result = await _dapper.QueryAsync<ProductoDto>(qry);
            return _mapper.Map<List<Producto>>(result);
        }

        public async Task<bool> CreateProducto(Producto producto)
        {
            string qry = @"INSERT INTO productos 
                 (NOMBRE, DESCRIPCION, PRECIO, DISPONIBLE, CATEGORIA_ID, TIEMPO_PREPARACION) 
                 VALUES (@NOMBRE, @DESCRIPCION, @PRECIO, @DISPONIBLE, @CATEGORIA_ID, @TIEMPO_PREPARACION)";
            ...
            return await _dapper.ExecuteAsync(qry, parameters) > 0;
        }

        public async Task<List<Pedido>> GetAllPedidos()
        {
            string qry = "SELECT * FROM pedidos";
            var result = await _dapper.QueryAsync<PedidoDto>(qry);
            return _mapper.Map<List<Pedido>>(result);
        }

        public async Task<int> CreatePedido(Pedido pedido)
        {
            string qry = @"INSERT INTO pedidos 
                 (CLIENTE_ID, ESTADO, TOTAL, COCINERO_ID, DOMICILIARIO_ID, INSTRUCCIONES) 
                 VALUES (@CLIENTE_ID, @ESTADO, @TOTAL, @COCINERO_ID, @DOMICILIARIO_ID, @INSTRUCCIONES)";
            ...
            var parameters = new
            {
                ...
                TOTAL = pedido.Total,
                COCINERO_ID = pedido.CocineroId,
                DOMICILIARIO_ID = pedido.DomiciliarioId,
        ...
        public async Task<bool> CreateDetallePedido(DetallePedido detalle)
        {
            string qry = @"INSERT INTO detalle_pedido 
                 (PEDIDO_ID, PRODUCTO_ID, CANTIDAD, PRECIO_UNITARIO, OBSERVACIONES) 
                 VALUES (@PEDIDO_ID, @PRODUCTO_ID, @CANTIDAD, @PRECIO_UNITARIO, @OBSERVACIONES)";
        ...
        {
            string qry = "SELECT * FROM metodos_pago";
            var result = await _dapper.QueryAsync<MetodoPagoDto>(qry);
            return _mapper.Map<List<MetodoPago>>(result);
        }

        public async Task<bool> CreatePago(Pago pago)
        {
            string qry = @"INSERT INTO pagos 
                 (PEDIDO_ID, METODO_PAGO_ID, MONTO, ESTADO, TRANSACCION_ID) 
                 VALUES (@PEDIDO_ID, @METODO_PAGO_ID, @MONTO, @ESTADO, @TRANSACCION_ID)";
